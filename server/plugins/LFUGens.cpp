...
struct EnvGen : public Unit {
    double m_a1, m_a2, m_b1, m_y1, m_y2, m_grow, m_level, m_endLevel, m_segResidual;
...
void Vibrato_Ctor(Vibrato* unit) {
    unit->mFreqMul = 4.0 * SAMPLEDUR;
    double initPhase = unit->mPhase = 4.0 * sc_wrap(ZIN0(7), 0.f, 1.f) - 1.0;
...
void Unwrap_Ctor(Unwrap* unit) {
    ...
    if (in < lo || in >= hi)
        offset = floor((lo - in) / unit->m_range) * unit->m_range;
    else
        offset = 0.f;
...
static inline bool EnvGen_nextSegment(EnvGen* unit, int& counter, double& level) {
    ...
    initSegment:
        ...
        double durInSamples = dur * SAMPLERATE + unit->m_segResidual;
        int32 durInSamples_floor = (int32)durInSamples;
        counter = sc_max(1, durInSamples_floor);
        unit->m_segResidual = durInSamples - counter;
