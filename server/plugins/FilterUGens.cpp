	...
	if ( (lagu == unit->m_lagu) && (lagd == unit->m_lagd) ) {
		LOOP1(inNumSamples,
			...
			if ( y1a > y1b )
				...
				y1b = y1a + b1d * (y1b - y1a);
			   printf("[Lag2] Lag2UD_next-unchanged: %f\n", y1b);//mtm
			ZXP(out) = y1b;
		);
	} else {
		unit->m_b1u = lagu == 0.f ? 0.f : exp(log001 / (lagu * unit->mRate->mSampleRate));
	...
	if ( (lagu == unit->m_lagu) && (lagd == unit->m_lagd) ) {
		LOOP1(inNumSamples,
			...
			if ( y1a > y1b ) {
			  ...
			  printf("[Lag3UD] next-unchanging: %f\n", y1c);//mtm
			ZXP(out) = y1c;
		);
	} else {
		unit->m_b1u = lagu == 0.f ? 0.f : exp(log001 / (lagu * unit->mRate->mSampleRate));
...
void Median_next(Median* unit, int inNumSamples)
{
	...
	LOOP1(inNumSamples,
		  float val = Median_InsertMedian(unit, ZXP(in));
		  printf("[Median] next_: %f\n", val);//mtm
		  ZXP(out) = val;
...
void Compander_next(Compander* unit, int inNumSamples)
{
	...
	LOOP1(inNumSamples,
		  float val = ZXP(in) * gain; gain += gain_slope;
		  printf("[Compander] next: %f\n", val);//mtm
		  ZXP(out) = val;
