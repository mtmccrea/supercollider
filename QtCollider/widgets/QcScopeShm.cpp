...
void QcScopeShm::updateElWarp()
{
    initMercatorPixDecoder(COLPIX * 2, COLPIX, _elWarp);
}

void QcScopeShm::resetPixmaps()
{
...
void QcScopeShm::paintAedaGrid( const QRect &area, QPainter & painter, bool overlay )
{
...
void QcScopeShm::paintAeda( int maxFrames, int availFrames, int drawCount,
...
void QcScopeShm::paintVectorScope(int maxFrames, int availFrames, int drawCount,
                                  const QRect &area, QPainter &painter)
{
...
void QcScopeShm::paintVectorGrid( const QRect &area, QPainter &painter)
{
    float minSize, halfMin;
    minSize = fmin(area.width(), area.height());
    halfMin = minSize * 0.5;
...
void QcScopeShm::storeCoeffs(int width, int height, const float pattern) {
...
void QcScopeShm::initMollweidePixDecoder(const int width, const int height) {
    decoderInitialized = false;
...
void QcScopeShm::initMercatorPixDecoder(const int width, const int height, const float elWarp) {
...
void QcScopeShm::paintMollweide(int maxFrames, int availFrames,
...
void QcScopeShm::paintMollweideGrid( const QRect &area, QPainter & painter ) {
...
void QcScopeShm::paintMercator(int maxFrames, int availFrames,
                               int drawCount, float ampShape, float normScale,
                               const QRect &area, QPainter & painter ) {
    int minSize  = qMin(area.width()/2, area.height());
    int minSize2 = minSize*2;
...
void QcScopeShm::paintMercatorGrid( const QRect &area, QPainter & painter ) {
    int minSize  = qMin(area.width()/2, area.height());
    int minSize2 = minSize*2;
...
//void QcScopeShm::lensDeform(const QPainterPath &source, const qreal m_radius, const QRectF &rect, const qreal deformAmt, QPainter &painter)
//{
//    QPainterPath path;
//    path.addPath( source );
//
...
void QcScopeShm::lensDeform(const QPainterPath &source, const qreal radius,
...
void QcScopeShm::prepPainter ( QPainter &p, QColor &color, bool fill) {
    if (fill) {
        p.setPen(Qt::NoPen);
        p.setBrush(color);
    } else {
